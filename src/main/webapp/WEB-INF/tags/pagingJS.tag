...
<c:if test="${searchCriteria.pageCount > 1}">
    ...
    <c:if test="${!(searchCriteria.page==0)}">
    	<a href="javascript:void(0)" onclick="return processReportPaging('<%=pagingLink%>?p=0');">First&nbsp;</a>
    	<a href="javascript:void(0)" onclick="return processReportPaging('<%=pagingLink%>?p=${searchCriteria.page-1}');">Previous&nbsp;</a>
    </c:if>
    <%
    ...
    %>
        <a href="javascript:void(0)" onclick="return processReportPaging('<%=pagingLink%>?p=<%=i%>');"><%=(i+1)%></a>
    <%
    }
    ...
    <c:if test="${!(searchCriteria.page==searchCriteria.pageCount-1)}">
    	<a href="javascript:void(0)" onclick="return processReportPaging('<%=pagingLink%>?p=${searchCriteria.page+1}');">&nbsp;Next</a>
    </c:if>
    <c:if test="${page + 20 < searchCriteria.pageCount}">
        <a href="javascript:void(0)" onclick="return processReportPaging('<%=pagingLink%>?p=${searchCriteria.pageCount-1}');">&nbsp;Last</a>
    </c:if>
</c:if>
</span>