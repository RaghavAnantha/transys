...
<c:if test="${searchCriteria.pageCount > 1}">
    ...
    <c:if test="${!(searchCriteria.page==0)}">
        <a href="<%=pagingLink%>?<%=(redirect == null?"":"redirect="+redirect+"&")%>p=0">First&nbsp;</a>
        <a href="<%=pagingLink%>?<%=(redirect == null?"":"redirect="+redirect+"&")%>p=${searchCriteria.page-1}">Previous&nbsp;</a>
    </c:if>
    <%
    ...
    %>
        <a href="<%=pagingLink%>?<%=(redirect == null?"":"redirect="+redirect+"&")%>p=<%=i%>"><%=(i+1)%></a>
    <%
    }
    ...
    <c:if test="${!(searchCriteria.page==searchCriteria.pageCount-1)}">
        <a href="<%=pagingLink%>?<%=(redirect == null?"":"redirect="+redirect+"&")%>p=${searchCriteria.page+1}">&nbsp;Next</a>
    </c:if>
    <c:if test="${page + 20 < searchCriteria.pageCount}">
        <a href="<%=pagingLink%>?<%=(redirect == null?"":"redirect="+redirect+"&")%>p=${searchCriteria.pageCount-1}">&nbsp;Last</a>
    </c:if>
</c:if>
</span>